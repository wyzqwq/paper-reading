...
const SYSTEM_PROMPT = [
  ...
  '3b. 独立成块的公式(display equation,常为多行,上下标常被换行切断如 "P\\nt mt"、"zfwd\\nt")必须整块合并为一条 sentence:en 逐字拷贝整个公式块的文字层文本(保留内部换行),zh 依页图重建为一条完整 $$...$$ LaTeX,不按行拆散、不拆成多条。',
  '4. 数学公式、变量符号(如 x²、α、∇)、代码、URL、参考文献编号 不翻译,只译周围文字。',
  '5. 已是中文的片段跳过不译,en 照抄、zh 留原样或空。',
  '6. 句子间顺序与原文一致;不要增删句子;不要加任何解释/前言/后语,只输出 JSON。',
  '7. 用户消息可能含 <page>本页文本</page> + <ctx_prev>上一页末尾</ctx_prev> / <ctx_next>下一页开头</ctx_next>。只翻译 <page> 内的文本(它才是当前页),<ctx_*> 的文字不得出现在 sentences 的 en 里。<page> 首部单独一行可能是上一页被切断句子的开头(与后续文字同属一句)——正常作一个完整句翻译即可,en 照常逐字拷贝 <page> 内文字(含该行)。若该句的续句与句首片段之间隔着页眉、图注等行:这些行不属于该句,各自单独成句翻译,不得拷入合并句的 en;续句也不得并入它前面紧邻的图注/页眉句子。<page> 结尾被切断的半句照常只译本页部分。',
